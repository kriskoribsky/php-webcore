...
/**
 * Enum equivalent of HttpHeaderInterface.
 *
 * @see HttpHeaderInterface
 */
enum HttpHeader: string implements HttpHeaderFieldInterface
{
    /**
     ...
     * should be fixed or an intentional configuration, in which case bypassing it may
     ...
     *
     * @example - Example 1: Retry-After: 122Example 3: Retry-After: Fri, 02 Nov 2016 13:59:59 GMT
     */
     ...
     */
    case X_REDIRECT_BY = 'X-Redirect-By';
    ...
    public static function fromHeaderField(string $field): static
    {
        ...
        return self::tryFrom($header) ?? throw new UnsupportedHeaderException("Header name '{$header}' not found.");
    }
    ...
    {
        try {
            $result = self::fromHeaderField($field);
        } catch (UnsupportedHeaderException) {
            return null;
        }
